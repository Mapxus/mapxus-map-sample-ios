  ...
  [self dismissViewControllerAnimated:YES completion:^{
    if (self.delegate && [self.delegate respondsToSelector:@selector(completeParamConfiguration:)]) {
      ...
      switch (weakSelf.searchTypeButton.selectedSegmentIndex) {
        case 0:
        {
          params[@"distanceSearchType"] = @(MXMSearchScopeType_Polygon);
        }
          break;
        case 1:
        {
          params[@"distanceSearchType"] = @(MXMSearchScopeType_Point);
        }
          break;
        case 2:
        {
          params[@"distanceSearchType"] = @(MXMSearchScopeType_Gate);
        }
          break;
        default:
          break;
