//
//  SearchPOIByIDParamViewController.m
//  MapxusMapSample
//
...
@interface SearchPOIByIDParamViewController () <UITableViewDelegate, UITableViewDataSource>
...
@implementation SearchPOIByIDParamViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.view.backgroundColor = [UIColor whiteColor];
    [self.dataSource addObject:@"12586"];
    [self layoutUI];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    POIIDCell *cell = [tableView dequeueReusableCellWithIdentifier:@"POIIDCell"];
    [cell refreshData:self.dataSource[indexPath.row]];
    __weak typeof(self) weakSelf = self;
    cell.endEditBlock = ^(POIIDCell * _Nonnull cell, NSString * _Nullable text) {
...
- (UITableView *)tableVeiw {
    if (!_tableVeiw) {
        ...
        [_tableVeiw registerClass:[POIIDCell class] forCellReuseIdentifier:@"POIIDCell"];
    }
    return _tableVeiw;
}
